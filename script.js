...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const formClassGroup = document.getElementById('formClassGroup'); // Container for FC selector
   ...
   const speciesMgmtFeedback = document.getElementById('speciesMgmtFeedback');
    ...
    const SETTINGS_STORAGE_KEY = 'timberTallySettings'; // New key for settings

    let currentLocation = null;
    let feedbackTimeout = null;
    let settingsFeedbackTimeout = null; // New feedback timeout
    ...
    // --- Settings State (New!) ---
    let currentBaf = 10; // Default BAF
    let currentLogRule = 'Doyle'; // Default Rule
    let currentFormClass = 78; // Default Form Class
   ...
   let currentSpeciesList = [...DEFAULT_SPECIES];
    ...
    const SCRIBNER_FC78_VOLUMES = {
        ...
        '20': { '1.0': 157, '1.5': 214, '2.0': 270, '2.5': 317, '3.0': 364, '3.5': 398, '4.0': 432, '4.5': 459, '5.0': 486 }, // Corrected 3.5 log based on likely progression
        ...
        '40': { '1.0': 703, '1.5': 988, '2.0': 1274, '2.5': 1532, '3.0': 1791, '3.5': 1993, '4.0': 2195, '4.5': 2384, '5.0': 2574, '5.5': 2768, '6.0': 2961 } // Corrected 5.5/6.0 keys
    };

    // International 1/4" FC78 Volume Table
    const INTERNATIONAL_FC78_VOLUMES = {
        ...
        '22': { '1.0': 211, '1.5': 289, '2.0': 357, '2.5': 434, '3.0': 499, '3.5': 552, '4.0': 593, '4.5': 646, '5.0': 681 }, // Corrected 3.0 log from image likely typo
        '23': { '1.0': 231, '1.5': 317, '2.0': 404, '2.5': 467, '3.0': 552, '3.5': 608, '4.0': 663, '4.5': 714, '5.0': 766 }, // Corrected 2.0 log from image likely typo
    ...
    // --- Settings Functions ---
    function showSettingsFeedback(message, isError = false, duration = 2000) { // Shorter duration
    ...
    // Function to update the current plot number based on the latest entry in collectedData
    ...
    function parseCsvAndLoadData(csvContent) {
        ...
        for (let i = headerIndex + 1; i < lines.length; i++) {
            ...
            if (values.length < reqCols) { console.warn(`Skipping malformed row ${i + 1}.`); continue; }
            try {
                ...
                const lat = latStr !== null ? parseFloat(latStr) : null; const lon = lonStr !== null ? parseFloat(lonStr) : null;
    ...
    function checkAndSetLogsForDbh() {
        ...
        } // Add checks for other rules if necessary
    }
    ...
    function renderEntries() {
        if (!entriesTableBody || !entryCountSpan || !noEntriesRow || !saveCsvBtn || !viewTallyBtn || !deleteAllBtn || !deleteBtn) return; // Safety check
        ...
        if (hasData) {
            for (let i = collectedData.length - 1; i >= 0; i--) {
                ...
                tr.innerHTML = `<td><input type="checkbox" data-id="${entry.id ?? `no-id-${i}`}"></td>
    ...
    function generateAndDownloadCsv() {
        ...
        try {
            ...
            link.setAttribute("href", url); link.setAttribute("download", `TimberTally_${ruleP}${fcP}_${bafP}_${ts}.csv`);
    ...
    if (deleteBtn) deleteBtn.addEventListener('click', () => {
        ...
        const idsToDelete = new Set(Array.from(checks).map(cb => parseInt(cb.getAttribute('data-id'), 10)).filter(id => !isNaN(id)));
        if (idsToDelete.size === 0) { showFeedback("Cannot identify entries.", true); return; } const num = idsToDelete.size; const word = num === 1 ? 'entry' : 'entries'; if (!confirm(`Delete ${num} selected ${word}?`)) return;
        collectedData = collectedData.filter(entry => !(entry && idsToDelete.has(entry.id))); renderEntries(); saveSessionData(); showFeedback(`${num} ${word} deleted.`);
    });

    if (deleteAllBtn) deleteAllBtn.addEventListener('click', () => {
        if (collectedData.length === 0) { showFeedback("No data to delete.", true); return; } if (!confirm('WARNING: Delete ALL collected data?')) return; collectedData = [];
    ...
    if(loadProjectBtn) loadProjectBtn.addEventListener('click', () => { if(!loadProjectSelect) return; const name = loadProjectSelect.value; if (!name) { showProjectFeedback("Select project.", true); return; } if (!savedProjects[name]) { showProjectFeedback(`Project "${name}" not found.`, true); return; } if (!confirm(`Load project "${name}"? Replaces current data.`)) return; try { collectedData = JSON.parse(JSON.stringify(savedProjects[name])); if(projectNameInput) projectNameInput.value = name; renderEntries(); saveSessionData(); showProjectFeedback(`Project "${name}" loaded.`); currentLocation = null; if(locationStatus)locationStatus.textContent = 'Location not set'; } catch (e) { showProjectFeedback(`Load Error: ${e.message}`, true); collectedData = []; renderEntries(); saveSessionData(); } });
    if(deleteProjectBtn) deleteProjectBtn.addEventListener('click', () => { if(!loadProjectSelect) return; const name = loadProjectSelect.value; if (!name) { showProjectFeedback("Select project.", true); return; } if (!savedProjects[name]) { showProjectFeedback(`Project "${name}" not found.`, true); return; } if (!confirm(`Delete project "${name}"?`)) return; delete savedProjects[name]; saveProjectsToStorage(); populateLoadProjectDropdown(); if (projectNameInput && projectNameInput.value === name) projectNameInput.value = ''; showProjectFeedback(`Project "${name}" deleted.`); });
    if(loadCsvBtn) loadCsvBtn.addEventListener('click', () => { if(!csvFileInput) return; const file = csvFileInput.files[0]; if (!file) { showProjectFeedback("Select CSV file.", true); return; } if (!file.name.toLowerCase().endsWith('.csv')) { showProjectFeedback("Must be .csv file.", true); csvFileInput.value = null; return; } const reader = new FileReader(); reader.onload = (e) => { try { const content = e.target.result; const parsed = parseCsvAndLoadData(content); if (parsed.length === 0) { showProjectFeedback("CSV parsed, no data found.", true); csvFileInput.value = null; return; } if (!confirm(`Load ${parsed.length} entries from "${file.name}"? Replaces current data.`)) { csvFileInput.value = null; return; } collectedData = parsed; if(projectNameInput) projectNameInput.value = file.name.replace(/\.csv$/i, ''); renderEntries(); saveSessionData(); showProjectFeedback(`Loaded ${parsed.length} from "${file.name}".`); currentLocation = null; if(locationStatus)locationStatus.textContent = 'Location not set'; csvFileInput.value = null; } catch (err) { showProjectFeedback(`CSV Load Error: ${err.message}`, true); csvFileInput.value = null; } }; reader.onerror = () => { showProjectFeedback("File read error.", true); csvFileInput.value = null; }; reader.readAsText(file); });

    // --- Compass Logic ---
    let orientationHandler = null;
    const handleOrientationEvent = (event) => { let h=null, s='---'; if(event.absolute===true && event.alpha!==null){h=360-event.alpha;s='True (abs)';} else if(event.webkitCompassHeading!==undefined && event.webkitCompassHeading!==null){h=event.webkitCompassHeading;s='Mag (webkit)';} else if(event.alpha!==null){h=360-event.alpha;s='Mag (alpha)';} if(h!==null){h=(h+360)%360; if(compassNeedle)compassNeedle.style.transform=`translateX(-50%) rotate(${h}deg)`; if(compassHeading)compassHeading.textContent=`${h.toFixed(0)}°`; if(compassSource)compassSource.textContent=` (${s})`;} else {if(compassHeading)compassHeading.textContent=`---°`; if(compassSource)compassSource.textContent=` (No reading)`;}};
    if(showCompassBtn){showCompassBtn.addEventListener('click',()=>{ if(typeof DeviceOrientationEvent!=='undefined'){if(typeof DeviceOrientationEvent.requestPermission==='function'){DeviceOrientationEvent.requestPermission().then(p=>{if(p==='granted'){startCompass();}else{alert("Compass needs orientation permission.");}}).catch(e=>{alert("Permission error: "+e.message);});}else{startCompass();}}else{alert("Compass features not supported.");}}); } else { console.error("showCompassBtn not found"); }
    function startCompass() { if(!compassContainer||!compassNeedle||!compassHeading||!compassSource){alert("Compass UI missing.");return;} compassContainer.style.display='flex';compassHeading.textContent=`---°`;compassSource.textContent=` (Wait...)`; orientationHandler=handleOrientationEvent; if('ondeviceorientationabsolute' in window){window.addEventListener('deviceorientationabsolute',orientationHandler,true);}else if('ondeviceorientation' in window){window.addEventListener('deviceorientation',orientationHandler,true);}else{alert("Orientation events unsupported.");compassHeading.textContent=`N/A`;compassSource.textContent=` (Unsupported)`;}}
    if(closeCompassBtn){closeCompassBtn.addEventListener('click',()=>{ if(!compassContainer) return; compassContainer.style.display='none'; if(orientationHandler){if('ondeviceorientationabsolute' in window)window.removeEventListener('deviceorientationabsolute',orientationHandler,true); if('ondeviceorientation' in window)window.removeEventListener('deviceorientation',orientationHandler,true); orientationHandler=null;}}); } else { console.error("closeCompassBtn not found"); }
    // --- End Compass Logic ---
    ...
    try {
        ...
        document.body.innerHTML = `<div style="padding: 20px; color: red; border: 2px solid red; margin: 20px; font-size: 1.2em;"><h2>App Init Failed</h2><p>Check console (F12).</p><p><strong>Error:</strong> ${initError.message}</p></div>`;
    }
